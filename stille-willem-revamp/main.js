...
let guild;
let botId;
let kokosnootRole, inhetzonnetjeRole, stamkroegverbodRole, emoteloosRole, makkersRole, internationalRole;
 ...
 * If the role is Kokosnoot or Stamkroegverbod, the freedom
...
setInterval(() => {
	...
	//log((new Date()).getTime());
		...
		default:
            ...
            schandpaalChannel.messages.fetch(item.announcement).then((message) => {
                message.delete().catch((err) => {
                    console.error(`Could not delete message with ID ${message.id}: `, err);
                });;
            }).catch((err) => {
...
function parseMessage(msg) {
	...
	if(validCommand) {
        ...
        if(getRandomInt(1, chance+2) === chance || (msg.member.hasPermission('MANAGE_ROLES'))) {
